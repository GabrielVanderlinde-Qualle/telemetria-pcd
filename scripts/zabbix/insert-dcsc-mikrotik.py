#!/usr/bin/env python3
"""
Script para criar hosts no Zabbix via API (Focado no Mikrotik SC)
Lê os dados do arquivo consulta_estacoes_SC.csv e envia para o Zabbix via SNMP.
"""
import json
import requests
import csv
...
def load_sc_hosts_from_csv(csv_filepath, device_type="mikrotik"):
    hosts = []
    ...
    with open(csv_filepath, mode='r', encoding='utf-8') as file:
        ...
        for row in reader:
            ...
            hostname = f"BR.SC._.DCSC.{codigo}.MIKROTIK"
            visible_name = f"{codigo} - Mikrotik ({nome_cidade})"
...
def main():
    ...
    print("=" * 60)
    print("Script de Criação de MIKROTIK DCSC (SC) via API lendo do CSV")
    print("=" * 60)
    ...
    try:
        group_ids = [zapi.get_hostgroup_ids(["BR.SC._.DCSC - PCDs"])[n] for n in ["BR.SC._.DCSC - PCDs"]]
        template_ids = [zapi.get_template_ids(["Mikrotik by SNMP"])[n] for n in ["Mikrotik by SNMP"]]
    except Exception as e:
        print(f"✗ Erro ao buscar grupos/templates de SC: {e}");
        return

    print(f"\nLendo dados do CSV {CSV_FILE}...")
    hosts_data = load_sc_hosts_from_csv(CSV_FILE, "mikrotik")
    print(f"✓ {len(hosts_data)} Mikrotiks preparados.")
    ...
    print("\n" + "=" * 60)
    print(f"✓ Mikrotiks SC criados/atualizados: {created}/{updated} | ⊘ Pulados: {skipped} | ✗ Erros: {errors}")
    print("=" * 60)
